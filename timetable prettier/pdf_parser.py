...
if __name__ == "__main__":
    # Test parsing
    result = parse_timetable_pdf("timetables/sec1.pdf")
    print(f"Section: {result['section']}")
    print(f"Max slot: {result['max_slot']}")
    print(f"Valid slots: {result['time_slots']}")
    print("\nTimetable:")
    for day, slots in result['timetable'].items():
        print(f"\n{day}:")
        for slot, info in slots.items():
            if info:
                print(f"  Slot {slot}: {info['name']} ({info['code']}) - {info['location']} - {info['lecturer']}")
